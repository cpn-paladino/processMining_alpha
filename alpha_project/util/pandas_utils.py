...
from alpha import constants
from util import xes_constants
...
def check_dataframe_columns(df):
    ...
    if len(set(df.columns).intersection(
        ...
        raise Exception(
            "please format your dataframe accordingly! df = format_dataframe(df, case_id='<name of the case ID column>', activity_key='<name of the activity column>', timestamp_key='<name of the timestamp column>')")
